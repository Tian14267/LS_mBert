...
from third_party.graph_utils import (
...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if addi_key is not None:  ### key_layer:(32,12,128,64)   addi_key:(32,1,128,64)
            assert addi_key.dim() == 4
            num_syn_head = addi_key.size(1)
            repeat_num = int(key_layer.size(1)/num_syn_head)
            assert key_layer.size(1)%num_syn_head == 0
            #temp_1 = key_layer[:, :num_syn_head]
            #key_layer[:, :num_syn_head] += addi_key
            addi_key_copy = addi_key.repeat_interleave(repeat_num, dim=1)
            key_layer = key_layer + addi_key_copy


        if addi_value is not None:  ##  value_layer:(32,12,128,64)   addi_value:(32,1,128,64)
            assert addi_value.dim() == 4
            num_syn_head = addi_value.size(1)
            repeat_num = int(key_layer.size(1)/num_syn_head)
            #value_layer[:, :num_syn_head] += addi_value
            addi_value_copy = addi_value.repeat_interleave(repeat_num, dim=1)
            value_layer = value_layer + addi_value_copy
...
class BertEncoder(nn.Module):
    ...
    def forward(
        ...
        if dist_mat is not None:
            ...
            gat_rep = self.gat_model(word_embeddings, dist_mat, dep_tag_ids, pos_tag_ids)
...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(
        ...
        #print(outputs_all.cpu().numpy())
        #for one in outputs_all:
        #    print(one.shape)
        #exit()

        outputs = outputs_all
        #input_tensor = outputs_all[1]
        #embedding_tensor = outputs_all[2]
        ...
        if self.use_structural_loss:
            ...
            struct_loss = self.dist_loss(dist_probe_out,
                          ...
                          self.depth_loss(depth_probe_out,
                                          ...
                                          length_batch=attention_mask.sum(1))[0]
            loss += self.loss_coeff * struct_loss
            outputs = (loss,) + outputs[1:]

        return (outputs,sequence_output)  # (loss), logits, (hidden_states), (attentions)
