...
from third_party.processors.utils_tag_codeswitch import (
...
from third_party.modeling_bert import BertForTokenClassification
...
def train(args, train_dataset, model, tokenizer, labels, pad_token_label_id, lang2id=None):
    ...
    for _ in train_iterator:
        epoch_iterator = tqdm(train_dataloader, desc="Train Iteration", disable=args.local_rank not in [-1, 0])
...
def load_and_cache_examples(args, tokenizer, labels, pad_token_label_id, mode, lang, lang2id=None, few_shot=-1):
    ...
    langs = lang.split(',')
    logger.info("all languages = {}".format('_'.join(langs)))
    features = []
    ...
    #############################
    if mode == "train":
        CT = ChangeTokensToMutilLanguage("./muse_dict_data/mutillang_map/", args.covert_rate)
    else:
        CT = None
    #############################
    ...
    for lg in langs:
        ...
        if os.path.exists(cached_features_file) and not args.overwrite_cache:
            ...
            features.extend(torch.load(cached_features_file))
        else:
            # data_file = os.path.join(args.data_dir, lg, "{}.{}".format(mode, suffix))
            data_file = os.path.join(args.data_dir, "{}-{}".format(mode, lg))
            ...
            if args.local_rank in [-1, 0]:
                logger.info(
                    "Saving features into cached file {}, len(features)={}".format(
                        cached_features_file, len(features_lg)))
...
def main():
    ...
    ## Required parameters
    parser.add_argument("--data_dir", default="./download/panx_udpipe_processed", type=str,
    ...
    parser.add_argument("--output_dir", default="./outputs/panx_paper", type=str,
                        help="The output directory where the model predictions and checkpoints will be written.")
    parser.add_argument("--task_name", default="panx", type=str,
                        help="The task name.")

    ## Other parameters
    parser.add_argument("--labels", default="./download/panx_udpipe_processed/label.txt", type=str,
                        help="Path to a file containing all labels. If not specified, NER/POS labels are used.")
    parser.add_argument("--covert_rate", default=0.5, type=float,
                        help="covert_rate")
    ...
    parser.add_argument("--predict_langs", type=str, default="en,ar,bg,de,el,es,fr,hi,ru,tr,ur,vi,ko,nl,pt", help="prediction languages")
    parser.add_argument("--valid_langs", type=str, default=None, help="validation languages separated by ','.")
    ...
    if os.path.exists(args.output_dir) and os.listdir(
        ...
        raise ValueError(
            "Output directory ({}) already exists and is not empty. Use --overwrite_output_dir to overcome.".format(
                args.output_dir))
    else:os.makedirs(args.output_dir, exist_ok=True)
    ...
    logging.basicConfig(handlers=[logging.FileHandler(log_file_path), logging.StreamHandler()],
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        with open(output_test_results_file, "a") as result_writer:
            for lang in args.predict_langs.split(','):
                if not (
                        os.path.exists(os.path.join(args.data_dir, "test-{}.jsonl".format(lang))) or
                        os.path.exists(os.path.join(args.data_dir, "test-{}.tsv".format(lang)))
                ):
    ...
    if args.do_predict_dev and args.local_rank in [-1, 0]:
        ...
        with open(output_test_results_file, "w") as result_writer:
            for lang in args.predict_langs.split(','):
                if not (
                        os.path.exists(os.path.join(args.data_dir, "dev-{}.jsonl".format(lang))) or
                        os.path.exists(os.path.join(args.data_dir, "dev-{}.tsv".format(lang)))
                ):
                    logger.info("Language {} does not exist".format(lang))
                    continue
