...
TOKENIZERS = {
    ...
    "syntax-xlmr": XLMRobertaTokenizer,
}
...
def panx_tokenize_preprocess(args):
    def _preprocess_one_file(infile, outfile, idxfile, tokenizer, max_len):
        ...
        exception_counter = 0
        with open(infile, "rt") as fin, open(outfile, "w") as fout, open(
                idxfile, "w"
        ) as fidx:
            for line in fin:
                line = line.strip()
                ...
                if (
                        current_subwords_len == 0 or current_subwords_len > max_seq_len
                ) and len(token) != 0:
                    token = tokenizer.unk_token
                    current_subwords_len = 1

                # adding current token exceeds max sequence length
                ...
                if (subword_len_counter + current_subwords_len) > max_seq_len:
                    ...
                    fout.write(f"{token}\t{label}\t{head}\n")
                    fidx.write(f"{idx}\n")
                    subword_len_counter += current_subwords_len

        print(f"{exception_counter} examples are truncated.")
        return 1

    model_type = args.model_type
    tokenizer = TOKENIZERS[model_type].from_pretrained(
    ...
    for lang in args.languages.split(","):
        ...
        for file in files:
            ...
            outfile = os.path.join(out_dir, "{}.{}".format(file, suffix))
            idxfile = os.path.join(out_dir, "{}.{}.idx".format(file, suffix))
            if os.path.exists(outfile) and os.path.exists(idxfile):
                print(f"{outfile} and {idxfile} exist")
            else:
                code = _preprocess_one_file(
                    infile, outfile, idxfile, tokenizer, args.max_len
                )
                if code > 0:
...
def udpos_preprocess(args):
    def _read_one_file(file):
        data = []
        sent, tag, lines, head, deptag = [], [], [], [], []
        for line in open(file, "r"):
            items = line.strip().split("\t")
            if len(items) != 10:
                empty = all(w == "_" for w in sent)
                if not empty:
                    data.append((sent, tag, head, deptag, lines))
                sent, tag, head, deptag, lines = [], [], [], [], []
            else:
                sent.append(items[1].strip())
                tag.append(items[3].strip())
                deptag.append(items[7].strip())
                head.append(int(items[6].strip()))
                lines.append(line.strip())
                assert len(sent) == int(items[0]), \
                    "line={}, sent={}, tag={}".format(line, sent, tag)
        return data
    ...
    def remove_empty_space(data):
        ...
        for split in data:
            new_data[split] = []
            for sent, tag, head, deptag, lines in data[split]:
                new_sent = ["".join(w.replace("\u200c", "").split(" ")) for w in sent]
                lines = [line.replace("\u200c", "") for line in lines]
                assert len(" ".join(new_sent).split(" ")) == len(tag)
                new_data[split].append((new_sent, tag, head, deptag, lines))
        return new_data
            ...
            if len(data[split]) > 0:
                ...
                if suffix == "mt":
                    ...
                    with open(prefix + ".tsv", "w") as fout:
                        for sidx, (sent, tag, head, deptag, _) in enumerate(data[split]):
                            for widx, (w, t, h, d) in enumerate(zip(sent, tag, head, deptag)):
                                newline = (
                                    ""
                                    if (sidx == len(data[split]) - 1)
                                       and (widx == len(sent) - 1)
                                    else "\n"
                                )
                                fout.write("{}\t{}\t{}\t{}{}".format(w, t, h, d, newline))
                                # if split == 'test':
    ...
    languages = \
        "af ar bg de el en es et eu fa fi fr he hi hu id it ja kk ko mr nl pt ru ta te th tl tr ur vi yo zh".split(" ")
    for root, dirs, files in os.walk(args.data_dir):
        ...
        for f in sorted(files):
            if f.endswith("conll"):
                ...
                print(
                    " - finish reading {}, {}".format(
                        file, [(k, len(v)) for k, v in data.items()]
                    )
                )
...
def xnli_preprocess(args):
    def _preprocess_file(infile, output_dir, split):
        ...
        for i, line in enumerate(open(infile, "r")):
            ...
            label = (
                "contradiction"
                if items[1].strip() == "contradictory"
                else items[1].strip()
            )
    ...
    def _preprocess_train_file(infile, outfile):
        with open(outfile, "w") as fout:
            ...
            for i, line in enumerate(open(infile, "r")):
                ...
                label = (
                    "contradiction"
                    if items[2].strip() == "contradictory"
                    else items[2].strip()
                )
                writer.writerow([sent1, sent2, label])
        print(f"finish preprocess {outfile}")

    train_langs = 'ar,bg,de,el,en,es,fr,hi,ru,tr,ur,vi,zh'.split(',')
    if not os.path.exists(args.output_dir):
        os.makedirs(args.output_dir)
    for l in train_langs:
        infile = os.path.join(args.data_dir, "XNLI-MT-1.0/multinli/multinli.train.{}.tsv".format(l))
        outfile = os.path.join(args.output_dir, "train-{}.tsv".format(l))
...
def tatoeba_preprocess(args):
    ...
    for sl3, sl2 in lang3_dict.items():
        if sl3 != "eng":
            ...
            gold_out = f"{args.output_dir}/{sl2}-en.en.gold"
            shutil.copy(src_file, src_out)
            tgts = [l.strip() for l in open(tgt_file)]
            idx = range(len(tgts))
            data = zip(tgts, idx)
            with open(tgt_out, "w") as ftgt, open(gold_out, "w") as fgold:
...
def mtop_preprocess(args):
    DOMAINS = [
        'alarm', 'calling', 'event', 'messaging', 'music', 'news',
        'people', 'recipes', 'reminder', 'timer', 'watcher'
    ]
    FIELD_NAMES = ['id', 'intent', 'slots', 'utterance', 'domain', 'language', 'compositional_rep', 'tokens']
            ...
            with open(infile, 'r') as f:
                for line in f:
                    ...
                    if not line:
                        continue
                    fields = line.split('\t')
                    intent = fields[1].split(':')[1]
                    slots = []
                    # slots field can be empty string
                    if len(fields[2]) > 0:
                        # slots are comma separated
                        # 4:20:SL:CONTACT,21:26:SL:TYPE_CONTENT,36:38:SL:RECIPIENT
                        _slots = fields[2].split(',')
                        for s in _slots:
                            if s:
                                # each slot has 4 fields, separated by ':'
                                # start_offset:end_offset:SL:slot_tag
                                _s = s.split(':')
                                #assert len(_s) == 4, (s, _s)
                                slots.append([int(_s[0]), int(_s[1]), _s[3]])

                    assert len(fields) == len(FIELD_NAMES)
                    try:
                        # obj = json.loads(fields[7])
                        obj = ast.literal_eval(fields[7])
                    except ValueError as e:
                        print(fields[7])
                        print(e)

                    tokens = []
                    for j, (w, span) in enumerate(zip(obj['tokens'], obj['tokenSpans'])):
                        tokens.append({
                            'token': w,
                            'token_range': (span['start'], span['start'] + span['length'])
                        })
                    # print(tokens)
                    ...
                    for j, tok in enumerate(tokens):
                        ...
                        tok_start, tok_end = tok['token_range']
                        # we match word offset with slot offsets to find proper tag
                        for slot_start, slot_end, slot_tag in slots:
                            # if a token starts at the slot start offset
                            if tok_start == slot_start:
                                bio_tags[j] = 'B-{}'.format(slot_tag)
                                total_slots += 1
                                break
                            # if a token starts after a slot's start offset
                            # but the token ends at or before the slot ends
                            elif tok_start > slot_start and tok_end <= slot_end:
                                # an I-{} tag must be preceded by a B-{} or I-{} tag
                                if bio_tags[j - 1] in ['B-{}'.format(slot_tag), 'I-{}'.format(slot_tag)]:
                                    bio_tags[j] = 'I-{}'.format(slot_tag)
                                break
    ...
    # infile contains one example per line (tab separated fields)
    # there must be len(FIELD_NAMES) fields in every line
    def vocab_process(data_dir):
        ...
        intent_vocab = set()
        slot_vocab = set()
        for lang in ['en', 'de', 'es', 'fr', 'hi', 'th']:
            for split in ['train', 'dev', 'test']:
                with open(os.path.join(data_dir, '{}-{}.jsonl'.format(split, lang)),
                          'r', encoding='utf-8') as f_r:
                    for line in f_r:
                        line = line.strip()
                        ex = json.loads(line)
                        intent_vocab.add(ex['intent_label'])
                        for slot in ex['slot_labels']:
                            slot_vocab.add(slot)

        with open(os.path.join(data_dir, intent_label_vocab), 'w', encoding='utf-8') as f_w1, \
                open(os.path.join(data_dir, slot_label_vocab), 'w', encoding='utf-8') as f_w2:
    ...
    for lang in ['en', 'de', 'es', 'fr', 'hi', 'th']:
        for split in ['train', 'eval', 'test']:
            convert(
                os.path.join(args.data_dir, lang, '{}.txt'.format(split)),
                os.path.join(
                    args.output_dir,
                    '{}-{}.jsonl'.format('dev' if split == 'eval' else split, lang)
                )
            )
    vocab_process(args.output_dir)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--task", default="panx", type=str, required=True, help="The task name"
    )
    parser.add_argument(
        "--model_name_or_path",
    ...
    parser.add_argument(
        "--max_len", default=512, type=int, help="the maximum length of sentences"
    )
    parser.add_argument(
        "--do_lower_case", action="store_true", help="whether to do lower case"
    )
    parser.add_argument("--cache_dir", default=None, type=str, help="cache directory")
    parser.add_argument("--languages", default="en", type=str, help="process language")
    parser.add_argument(
        "--remove_last_token",
        action="store_true",
